...
public class ChatFileServiceTests
{
    ...
    public void GetFiles_ReturnsEmptyWhenDirectoryMissing()
    {
        ...
        try
        {
            ...
            var service = new ChatFileService();
            var files = service.GetFiles(project, "session");
    ...
    public async Task AddFileAsync_CopiesFileAndGeneratesUniqueNames()
    {
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(sourceFile, "chat-content");

            var service = new ChatFileService();
            var first = await service.AddFileAsync(project, "session", sourceFile);
            var second = await service.AddFileAsync(project, "session", sourceFile);
    ...
    public async Task DeleteFileAsync_RemovesFile()
    {
        ...
        try
        {
            ...
            var service = new ChatFileService();
            var uploaded = await service.AddFileAsync(project, "session", sourceFile);
            Assert.True(File.Exists(uploaded.FullPath));

            await service.DeleteFileAsync(project, "session", uploaded.Name);
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "f9-chat-tests-" + Guid.NewGuid().ToString("N"));
